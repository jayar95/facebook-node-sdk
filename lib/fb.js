...
var Facebook = (_class = function () {
  ...
  */
   ...
   */
		...
		value: function value(path, method, params, cb) {
			...
			(0, _request2.default)(requestOptions, function (error, response, body) {
				...
				var appUsage = parseResponseHeaderAppUsage(response.headers);
				if (appUsage !== null) {
					_this._appUsage = buildRateLimitObjectFromJson(appUsage);
				} else {
					_this._appUsage = emptyRateLimit;
				}

				var pageUsage = parseResponseHeaderPageUsage(response.headers);
				if (pageUsage !== null) {
					_this._pageUsage = buildRateLimitObjectFromJson(pageUsage);
				} else {
					_this._pageUsage = emptyRateLimit;
				}
   ...
   */
		...
		key: 'getAppUsage',
		value: function getAppUsage() {
			return this._appUsage;
		}
	}, {
		key: 'getPageUsage',
		value: function getPageUsage() {
